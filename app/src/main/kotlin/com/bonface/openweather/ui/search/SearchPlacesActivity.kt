...
import com.bonface.openweather.domain.model.SearchResult
...
class SearchPlacesActivity : AppCompatActivity() {
    ...
    private fun getWeatherForSelectedLocation(location: Location, searchResult: SearchResult) {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                weatherViewModel.uiState.collect { result ->
                    when (result) {
                        is WeatherUiState.Success -> {
                            hideLoading()
                            result.weather?.let { showBottomSheet(it) }
                        }
    ...
    private fun showBottomSheet(weather: WeatherEntity) {
    ...
    private fun getLocationGeoCode(searchResult: SearchResult) {
        try {
            ...
            if (address != null) {
                ...
                getWeatherForSelectedLocation(location, searchResult)
            }
